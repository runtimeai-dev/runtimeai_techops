...
echo "Logging in..."
LOGIN_RESP=$(curl -v -c $COOKIE_JAR -s -X POST "$API_URL/api/auth/login" -H "Content-Type: application/json" -d '{"tenant_id":"${TENANT_ID:-bank-a}", "email":"a-admin@${TENANT_ID:-bank-a}.local", "password":"password123"}')
...
PASSED=true
for i in {1..12}; do
    STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BUNDLE_CACHE_URL/quota/check" -H "Content-Type: application/json" -d '{"tenant_id":"${TENANT_ID:-bank-a}", "quota_type":"api_calls", "increment":1}')
